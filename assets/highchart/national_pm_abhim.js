let totalExpenitureMarginpmabhimsss =
    window.innerWidth > 768 && window.innerWidth < 1299 ? -150 : 0;
let expenditureHeightpmabhimsss = window.innerWidth > 768 && window.innerWidth < 1360 ? 200 : 250;
let expenditurPercentageeHeightpmabhimsss = window.innerWidth > 768 && window.innerWidth < 1360 ? 250 : 200;

let expenditureTitleYpmabhimsss =
...
let expenditureSubtitleYpmabhimsss =
...
$(document).ready(function(){
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#national-pmabhimsss-actualExpenditureTotal").text(data.totalArray.actualExpenditureTotal);
            $("#national-pmabhimsss-unspentBalance31stTotal").text(data.totalArray.unspentBalance31stTotal);
            ...
            nationalPmabhimsssUcFormTotalChart(UcUploadDetails);
        }
    });
});
...
$(document).ready(function(){
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/pmabhimsss-soe-expenditure-filter",
        success: function(data) {
            var programWiseExpenditure = data.yearlySoeDetails;
            PmabhimsssSoeExpenditure(programWiseExpenditure);         
        }
    });
});
$(document).on('change', '.pmabhimsss_card', function() {
    let financialYear = $('#pmabhimsss-year').find(":selected").val();
    const month = $('#pmabhimsss-month').find(":selected").val();
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/pmabhimsss-filter-dashboard",
        data: {
...
$(document).on('change', '.pmabhim_national_ucForm_filter', function() {
    var pmabhimnationalUcformFy = $('#pmabhim-national-ucform-fy').find(":selected").val();
    const pmabhimlabsnationalProgramUcForm = $('#pmabhim-national-institute-ucform').find(":selected").val();
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/pmabhimsss-filter-uc-form-dashboard",
        data: {
            'pmabhimNationalUcformFy': pmabhimnationalUcformFy,
            'pmabhimNationalInstituteUcForm' : pmabhimlabsnationalProgramUcForm
        },
        success: function(data) {           
            var UcUploadDetails = data.UcUploadDetails;
            nationalPmabhimsssUcFormTotalChart(UcUploadDetails);      
...
// Yearly SOE Expenditure under NOHPPCZ RC’s
$(document).on('change', '.pmabhim_yearly_soe_expenditure', function() {
...
function nationalPmabhimsssTotalChart(percentageExpenditure,percentageUnspentBalance,totalExpenditure,totalUnspentBalance,totalcommittedLiabilities,programHeadDetails){
    Highcharts.chart("national-pmabhimsss-total-expenditure-lac", {
        chart: {
            plotBackgroundColor: null,
            height: expenditureHeightpmabhimsss,
            margin: [0, 0, 0, 0],
            spacingTop: 0,
            spacingBottom: 0,
            spacingLeft: 0,
    ...
    Highcharts.chart("national-pmabhimsss-total-unspent-lac", {
        chart: {
            plotBackgroundColor: null,
            height: expenditureHeightpmabhimsss,
            margin: [0, 0, 0, 0],
    ...
    let overallChart = Highcharts.chart(
        "integrated-dashboard-chart-overall-program-expenditure-amount-pmabhimsss",
        {
            chart: {
                type: "pie",
    ...
    Highcharts.chart("national_expenditure_percentage_pmabhimsss", {
        chart: {
            plotBackgroundColor: null,
            height: expenditurPercentageeHeightpmabhimsss,
            margin: [0, 0, 0, 0],
            spacingTop: 0,
            spacingBottom: 0,
            spacingLeft: 0,
            spacingRight: 0,
            marginTop: totalExpenitureMarginpmabhimsss,
        },
        credits: {
            enabled: false,
        },
    ...
    Highcharts.chart("national_fund_unspent_percentage_pmabhimsss", {
        chart: {
            plotBackgroundColor: null,
            height: expenditurPercentageeHeightpmabhimsss,
            margin: [0, 0, 0, 0],
    ...
    Highcharts.chart("national_interest_earned_cy_percentage_pmabhimsss", {
        chart: {
            plotBackgroundColor: null,
            plotBackgroundColor: null,
            height: expenditurPercentageeHeightpmabhimsss,
...
function nationalPmabhimsssUcFormTotalChart(UcUploadDetails){
    ...
    Highcharts.chart('national-pmabhim-uc-upload-dashboard-Months-bar', {
        chart: {
            type: 'column'
        },
        title: {
...
function PmabhimsssSoeExpenditure(programWiseExpenditure){
    Highcharts.chart("national_expnediture_pmabhimsss", {
        chart: {
            type: "column",
        },
        title: {
    ...
    Highcharts.chart("national_instiute_wise_yearly_pmabhimsss", {
        chart: {
            type: "column",
        },
        title: {
...
$(document).ready(function() {
    ...
    var nationalDataTable = $('.pmabhimsss_national_uc_filter_datatable').DataTable({
        pageLength: 5,
        dom: 'Bfrtip',
        buttons: [
            {
    ...
    function fetchData() {       
        ...
        $.ajax({
            type: "GET",
            url: BASE_URL + 'national-users/pmabhimsss-dashboard-report',
            data: {
    ...
    $(document).on('click', '#pmabhimsss_form_type_uc_list', function(event) {
