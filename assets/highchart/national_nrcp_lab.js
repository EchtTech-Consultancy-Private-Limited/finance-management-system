let totalExpenitureMarginNrcpLab =
    window.innerWidth > 768 && window.innerWidth < 1299 ? -150 : 0;
let expenditureHeightNrcpLab = window.innerWidth > 768 && window.innerWidth < 1360 ? 200 : 250;
let expenditurPercentageeHeightNrcpLab = window.innerWidth > 768 && window.innerWidth < 1360 ? 250 : 200;

let expenditureTitleYNrcpLab =
...
let expenditureSubtitleYNrcpLab =
...
$(document).ready(function(){
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/nrcplab-filter-dashboard",
        success: function(data) {
            $('#national-nrcplab-unspentBalance1stTotal').text(data.totalArray.unspentBalance1stTotal)    
            $("#national-nrcplab-giaReceivedTotal").text(data.totalArray.giaReceivedTotal);
            $("#national-nrcplab-committedLiabilitiesTotal").text(data.totalArray.committedLiabilitiesTotal);
            $("#national-nrcplab-totalBalanceTotal").text(data.totalArray.totalBalanceTotal);
            $("#national-nrcplab-actualExpenditureTotal").text(data.totalArray.actualExpenditureTotal);
            $("#national-nrcplab-unspentBalance31stTotal").text(data.totalArray.unspentBalance31stTotal);
            ...
            nationalNrcplabTotalChart(percentageExpenditure,percentageUnspentBalance,totalExpenditure,totalUnspentBalance,totalcommittedLiabilities,programHeadDetails);         
            nationalNrcplabUcFormTotalChart(UcUploadDetails);
        }
    });
});
...
$(document).ready(function(){
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/nrcplab-soe-expenditure-filter",
        success: function(data) {
            var programWiseExpenditure = data.yearlySoeDetails;
            nrcplabSoeExpenditure(programWiseExpenditure);         
        }
    });
});
 $(document).on('change', '.nohppcz_sss_card', function() {
    let financialYear = $('#nrcplab-year').find(":selected").val();
    const month = $('#nrcplab-month').find(":selected").val();
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/nrcplab-filter-dashboard",
        data: {
...
$(document).on('change', '.nrcplab_national_ucForm_filter', function() {
    ...
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/nrcplab-filter-uc-form-dashboard",
        data: {
...
function nationalNrcplabTotalChart(percentageExpenditure,percentageUnspentBalance,totalExpenditure,totalUnspentBalance,totalcommittedLiabilities,programHeadDetails){
    Highcharts.chart("national-nrcplab-total-expenditure-lac", {
        chart: {
            plotBackgroundColor: null,
            height: expenditureHeightNrcpLab,
            margin: [0, 0, 0, 0],
            spacingTop: 0,
            spacingBottom: 0,
            spacingLeft: 0,
    ...
    Highcharts.chart("national-nrcplab-total-unspent-lac", {
        chart: {
            plotBackgroundColor: null,
            height: expenditureHeightNrcpLab,
            margin: [0, 0, 0, 0],
    ...
    Highcharts.chart("national_expenditure_percentage_nrcplab", {
        chart: {
            plotBackgroundColor: null,
            height: expenditurPercentageeHeightNrcpLab,
            margin: [0, 0, 0, 0],
            spacingTop: 0,
            spacingBottom: 0,
            spacingLeft: 0,
            spacingRight: 0,
            marginTop: totalExpenitureMarginNrcpLab,
        },
        credits: {
            enabled: false,
        },
...
function nationalNrcplabUcFormTotalChart(UcUploadDetails){
    ...
    Highcharts.chart('national-nrcplab-uc-upload-dashboard-Months-bar', {
        chart: {
            type: 'column'
        },
        title: {
...
$(document).ready(function() {
    ...
    var nationalDataTable = $('.nrcplab_national_uc_filter_datatable').DataTable({
        pageLength: 5,
        dom: 'Bfrtip',
        buttons: [
            {
    ...
    function fetchData() {       
        ...
        $.ajax({
            type: "GET",
            url: BASE_URL + 'national-users/nrcplab-dashboard-report',
            data: {
    ...
    $(document).on('click', '#nrcplab_form_type_uc_list', function(event) {
