...
$(document).ready(function(){
    $.ajax({
        ...
        success: function(data) {
            $("#national-ppcllab-unspentBalance1stTotal").text(data.totalArray.unspentBalance1stTotal);     
...
$(document).on('change', '.ppcllab_national_ucForm_filter', function() {
    var ppcllabsnationalUcformFy = $('#ppcl-national-ucform-fy').find(":selected").val();
    const ppcllabsnationalProgramUcForm = $('#ppcllab-national-institute-ucform').find(":selected").val();
    $.ajax({
        type: "GET",
        url: BASE_URL + "national-users/ppcllab-filter-uc-form-dashboard",
        data: {
            'ppcllabsNationalUcformFy': ppcllabsnationalUcformFy,
            'ppcllabsNationalInstituteUcForm' : ppcllabsnationalProgramUcForm
        },
        success: function(data) {           
            var UcUploadDetails = data.UcUploadDetails;
            nationalPpcllabUcFormTotalChart(UcUploadDetails);      
...
function nationalPpcllabUcFormTotalChart(UcUploadDetails){
    ...
    Highcharts.chart('national-ppcllab-uc-upload-dashboard-Months-bar', {
        chart: {
            type: 'column'
        },
        title: {
