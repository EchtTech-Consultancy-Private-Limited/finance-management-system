...
class PMABHIMSSSController extends Controller
{
    ...
    public function index()
    {
        ...
        $institutes = Institute::where('program_id', 5)->get();
        $sorUcLists = SOEUCUploadForm::with('users')->where('program_id', 5)->orderBy('id','desc')->get();
        ...
        return view('national-user.PM-ABHIM-SSS-dashboard', compact('totalArray','sorUcLists','institutePrograms','institutes','months'));
    }
     ...
     * @return void
     */
    public function pmabhimsssNationalFilterDdashboard(Request $request)
    {
        ...
        $instituteProgram = InstituteProgram::where('id', 5)->first();
        ...
        $query = SOEUCUploadForm::where('financial_year', $financialYear)->where('program_id', 5);
        $UcUploadDetails = [];
     ...
     * @return void
     */
    public function pmabhimsssNationalFilterUcFormDashboard(Request $request)
    {
        $query = SOEUCUploadForm::query();        
        if ($request->has('pmabhimNationalUcformFy') && $request->pmabhimNationalUcformFy) {
            $financialYear = $request->pmabhimNationalUcformFy;
        } else {
            $financialYear = Carbon::now()->year;
        }
        if ($request->has('pmabhimNationalInstituteUcForm') && $request->pmabhimNationalInstituteUcForm) {
            $query->where('institute_id', $request->pmabhimNationalInstituteUcForm);
        }
        $query->where('financial_year', $financialYear)->where('program_id', 5);
     ...
     * @return void
     */
    public function pmabhimsssSoeExpenditureFilter(Request $request)
    {
     ...
     * @return void
     */
    public function pmabhimsssDashboardReport(Request $request)
    {
        $fileName = '';
        $arrays = [];
        switch ($request->modulename) {
            case '1':
                $fileName = 'SOEUForm';
                $query = SOEUCForm::with('states','instituteProgram','SoeUcFormCalculation')->where('program_id', 5);
                break;
            case '2':
                $fileName = 'UCUpload';
                $query = SOEUCUploadForm::with('program')->where('program_id', 5);
                break;
            default:
