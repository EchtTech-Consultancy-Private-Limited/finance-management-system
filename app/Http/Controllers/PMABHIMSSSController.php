...
class PMABHIMSSSController extends Controller
{
    ...
    public function pmabhimsssNationalFilterDdashboard(Request $request)
    {
        ...
        $UcUploadCount = SOEUCUploadForm::where('program_id', 5)->count();
        $UcUploadApproved = SOEUCUploadForm::where(['status' => '1', 'program_id' => 5])->count();
        $UcUploadNotApproved = SOEUCUploadForm::where(['status' => '2', 'program_id' => 5])->count();
        $UcUploadDetails = [
