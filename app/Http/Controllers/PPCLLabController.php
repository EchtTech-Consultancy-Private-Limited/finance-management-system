...
class PPCLLabController extends Controller
{
    ...
    public function index()
    {
        ...
        $institutes = Institute::where('program_id', 4)->get();
        $sorUcLists = SOEUCUploadForm::with('users')->where('program_id', 4)->orderBy('id','desc')->get();
        ...
        return view('national-user.PPCL-Lab', compact('totalArray','sorUcLists','institutePrograms','institutes','months'));
    }
     ...
     * @return void
     */
    public function ppcllabNationalFilterDdashboard(Request $request)
    {
     ...
     * @return void
     */
    public function ppcllabSoeExpenditureFilter(Request $request)
    {
     ...
     * @return void
     */
    public function ppcllabDashboardReport(Request $request)
    {
        $fileName = '';
        $arrays = [];
        switch ($request->modulename) {
            case '1':
                $fileName = 'SOEUForm';
                $query = SOEUCForm::with('states','instituteProgram','SoeUcFormCalculation')->where('program_id', 4);
                break;
            case '2':
                $fileName = 'UCUpload';
                $query = SOEUCUploadForm::with('program')->where('program_id', 4);
                break;
            default:
