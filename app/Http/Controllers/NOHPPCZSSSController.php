...
class NOHPPCZSSSController extends Controller
{
    ...
    public function index()
    {
        ...
        $institutes = Institute::where('program_id', 2)->get();
        $sorUcLists = SOEUCUploadForm::with('users')->where('program_id', 2)->orderBy('id','desc')->get();
        ...
        return view('national-user.NOHPPCZ-SSS-dashboard', compact('totalArray','sorUcLists','institutePrograms','institutes','months'));
    }
     ...
     * @return void
     */
    public function nohppczrsssNationalFilterDdashboard(Request $request)
    {
        ...
        $instituteProgram = InstituteProgram::where('id', 2)->first();
        ...
        $query = SOEUCUploadForm::where('financial_year', $financialYear)->where('program_id', 2);
        $UcUploadDetails = [];
     ...
     * @return void
     */
    public function nohppczrsssNationalFilterUcFormDashboard(Request $request)
    {
        $query = SOEUCUploadForm::query();
        if ($request->has('nohppczrcsssNationalUcformFy') && $request->nohppczrcsssNationalUcformFy) {
            $financialYear = $request->nohppczrcsssNationalUcformFy;
        } else {
            $financialYear = Carbon::now()->year;
        }
        if ($request->has('nohppczrcsssNationalInstituteUcForm') && $request->nohppczrcsssNationalInstituteUcForm) {
            $query->where('institute_id', $request->nohppczrcsssNationalInstituteUcForm);
        }      
        
        $query->where('financial_year', $financialYear)->where('program_id', 2);
        $UcUploadDetails = [];
     ...
     * @return void
     */
    public function nohppczrsssSoeExpenditureFilter(Request $request)
    {
     ...
     * @return void
     */
    public function nohppczrsssDashboardReport(Request $request)
    {
        $fileName = '';
        $arrays = [];
        switch ($request->modulename) {
            case '1':
                $fileName = 'SOEUForm';
                $query = SOEUCForm::with('states','instituteProgram','SoeUcFormCalculation')->where('program_id', 2);
                break;
            case '2':
                $fileName = 'UCUpload';
                $query = SOEUCUploadForm::with('program')->where('program_id', 2);
                break;
            default:
